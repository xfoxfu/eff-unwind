...
class AskHandler : public eff::flat_handler<void, Ask> {
 public:
  AskHandler(int id) : id(id) {}

 private:
  int id;
  void handle_command(Ask p, eff::resumption<void(int)> r) override {
    return std::move(r).tail_resume(id);
  }
};
